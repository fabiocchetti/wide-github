document.addEventListener('DOMContentLoaded', function() {
  ...
  browser.storage.sync.get('wideEnabled').then(function(result) {
    toggle.checked = result.wideEnabled !== false;
    updateToggleLabel();
  });
  
  // Initialize domain list
  browser.storage.sync.get('githubDomains').then(function(result) {
  ...
  toggle.addEventListener('change', function() {
    ...
    browser.storage.sync.set({ wideEnabled: enabled });
    updateToggleLabel();
    ...
    browser.tabs.query({ active: true, currentWindow: true }).then(function(tabs) {
      if (tabs[0]) {
        browser.storage.sync.get('wideEnabled').then(function(result) {
          browser.tabs.sendMessage(tabs[0].id, { wideEnabled: result.wideEnabled !== false });
        });
      }
    });
  });
  ...
  function addDomain() {
    ...
    browser.storage.sync.get('githubDomains').then(function(result) {
      ...
      if (!domainExists) {
        ...
        browser.storage.sync.set({ githubDomains: domains }).then(function() {
  ...
  function renderDomainList(domains) {
    ...
    domains.forEach(function(domain) {
      ...
      deleteBtn.addEventListener('click', function() {
        ...
        browser.storage.sync.set({ githubDomains: newDomains }).then(function() {
